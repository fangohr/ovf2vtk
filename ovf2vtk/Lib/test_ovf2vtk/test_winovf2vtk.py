...
def test_winovf2vtk_keys_no_parameters():
    ...
    for val in range(len(keys)):
        # compute actual result
        command = 'python winovf2vtk_new' + " {}".format(keys[val])
        p = subprocess.Popen(command, stdout=subprocess.PIPE,
                             stderr=subprocess.STDOUT, shell=True)
        doc = p.stdout.readlines()
        # remove '\r\n' characters
...
def test_winovf2vtk_example_cmd_lines():
    ...
    examples = "python winovf2vtk_new -V --ascii {} {}"\
        .format(infiles[0], outfiles[0]),\
        "python winovf2vtk_new --datascale=0.5 --posscale 1.0 \
--add Ms -a divrot --add yz -v {} {}".format(infiles[6], outfiles[6]),\
        "python winovf2vtk_new -h --binary {} {}"\
        .format(infiles[-1], outfiles[-1]),\
        "python winovf2vtk_new --add all --verbose --ascii \
--surface-effects {} {}"\
        .format(infiles[1], os.path.join('..', 'Examples', 'example.vtk')),\
        "python winovf2vtk_new -a Mx --add My -b \
...
def test_winovf2vtk_data():
    ...
    for i in range(len(files)):
            subprocess.Popen(("python winovf2vtk_new" + cmds[i] + " " +
                             files[i] + " " + vtk_files[i]),
